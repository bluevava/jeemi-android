...
func ApplyForPlatform(configurationYAML []byte, preferences runtimeconfig.Preferences, goos string) ([]byte, error) {
	...
	tunPath := "/tun/device"
	var routeExclusions *yaml.Node
	if goos == "android" {
		// Only validated route exclusions may survive. Android owns all other
		// tunnel fields, including its descriptor and route installation.
		if node, found, _ := document.Find(root, "/tun/route-exclude-address"); found {
			routeExclusions = node
		}
		tunPath = "/tun"
	}
	if err := document.DeleteMappingPath(root, tunPath); err != nil {
		return nil, err
	}
	if routeExclusions != nil {
		if err := document.SetMappingPath(root, "/tun/route-exclude-address", routeExclusions); err != nil {
			return nil, err
		}
	}
	...
	if goos != "android" {
		values = append(values, struct {
			path  string
			value *yaml.Node
		}{"/tun/enable", boolNode(preferences.ProxyMode == runtimeconfig.ProxyModeTUN)},
			struct {
				path  string
				value *yaml.Node
			}{"/tun/stack", stringNode(preferences.TUNStack)},
			struct {
				path  string
				value *yaml.Node
			}{"/tun/auto-route", boolNode(preferences.TUNAutoRoute)},
			struct {
				path  string
				value *yaml.Node
			}{"/tun/auto-detect-interface", boolNode(preferences.TUNAutoDetectInterface)})
	}
	...
	if deviceName := runtimeconfig.TUNDeviceNameForOS(goos); deviceName != "" && goos != "android" {
