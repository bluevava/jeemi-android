...
// earlier reloads. Rewritten paths are private to the actual session copy.
func (s *Session) Prepare(configuration string) error {
	...
	for _, item := range entries {
		...
		// Only ordinary provider files may seed a cache; never private session inputs.
		if e != nil && item.previous != "" {
			...
			if namespace != "generations" && namespace != "resolved" && namespace != "config.yaml" && namespace != "cache.db" && !strings.HasPrefix(previous, ".") {
		...
		if e == nil {
			...
			if total <= maxSessionBytes {
				e = atomicCopy(root, name, contextReader{ctx, source}, info.Size(), info.ModTime())
			}
		}
		source.Close()
		if e != nil || total > maxSessionBytes {
			log.Print("Jeemi: provider cache restore skipped")
		}
	}
